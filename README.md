...
> У меня есть версия этого проекта на form requests. Но она требует header Accept:application/json иначе может вернуть html страницу 404. Я не уверен что надо. А я пытался сделать что бы работало без всяких header как чистый запрос. Я не знаю как вы это тестировать будете. Поэтому взял рабочий но чуть хуже вариант 
